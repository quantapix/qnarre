...
fn file_write(relative_to_workspace_root: impl AsRef<Path>, content: Stream) -> Result<()> {
...
mod gen {
    ...
    pub fn traverse(defs: &Defs, node: fn(&mut Stream, &mut Stream, &Node, &Defs)) -> (Stream, Stream) {
        let mut types = defs.nodes.clone();
...
mod snapshot {
    ...
    fn expand_impl_body(defs: &Defs, node: &Node, name: &str, val: &Operand) -> Stream {
        let ident = Ident::new(&node.ident, Span::call_site());
        match &node.data {
            ...
            Data::Enum(variants) => {
                let arms = variants.iter().map(|(v, fields)| {
                    ...
                    if fields.is_empty() {
                        ...
                        let format = expand_impl_body(defs, lookup_node(defs, inner), &path, &Borrow(quote!(_val)));
                        quote! {
    ...
    fn expand_impl(defs: &Defs, node: &Node) -> Stream {
        let ident = Ident::new(&node.ident, Span::call_site());
        let body = expand_impl_body(defs, node, &node.ident, &Own(quote!(self.value)));
        let formatter = match &node.data {
            Data::Enum(variants) if variants.is_empty() => quote!(_formatter),
            _ => quote!(formatter),
        };
