...
mod expr {
    ...
    use syn::{
        token, Arm, Attribute, BinOp, Block, Expr, ExprArray, ExprAssign, ExprAsync, ExprAwait, ExprBinary, ExprBlock,
        ...
        ExprUnsafe, ExprWhile, ExprYield, FieldValue, Index, Label, Member, RangeLimits, ReturnType, Stmt, Token, UnOp,
    };
    impl Printer {
...
mod generics {
    ...
    use std::ptr;
    use syn::{
        BoundLifetimes, ConstParam, GenericParam, Generics, LifetimeParam, PredicateLifetime, PredicateType,
...
mod item {
    ...
    use proc_macro2::TokenStream;
    use syn::{
        Fields, FnArg, ForeignItem, ForeignItemFn, ForeignItemMacro, ForeignItemStatic, ForeignItemType, ImplItem,
        ImplItemConst, ImplItemFn, ImplItemMacro, ImplItemType, Item, ItemConst, ItemEnum, ItemExternCrate, ItemFn,
        ItemForeignMod, ItemImpl, ItemMacro, ItemMod, ItemStatic, ItemStruct, ItemTrait, ItemTraitAlias, ItemType,
        ItemUnion, ItemUse, Receiver, Signature, StaticMutability, TraitItem, TraitItemConst, TraitItemFn,
        TraitItemMacro, TraitItemType, Type, UseGlob, UseGroup, UseName, UsePath, UseRename, UseTree, Variadic,
    };
    impl Printer {
        ...
        fn item_verbatim(&mut self, tokens: &TokenStream) {
            use syn::parse::{Parse, ParseStream, Result};
            use syn::punctuated::Punctuated;
            use syn::{braced, parenthesized, token, Attribute, Generics, Ident, Lifetime, Token, Visibility};
            use verbatim::{
                FlexibleItemConst, FlexibleItemFn, FlexibleItemStatic, FlexibleItemType, WhereClauseLocation,
            };
            enum ItemVerbatim {
                Empty,
                Ellipsis,
            ...
            impl Parse for ItemVerbatim {
                fn parse(input: ParseStream) -> Result<Self> {
                        ...
                        || lookahead.peek(Token![impl])
                    {
                        ...
                        let negative_impl = !input.peek2(token::Brace) && input.parse::<Option<Token![!]>>()?.is_some();
    ...
    mod verbatim {
        ...
        use syn::{
            braced, token, Attribute, Block, Expr, Generics, Ident, Signature, StaticMutability, Stmt, Token, Type,
...
mod mac {
        ...
        pub fn macro_rules_tokens(&mut self, stream: TokenStream, matcher: bool) {
            ...
            for tt in stream {
                ...
                let (needs_space, next_state) = match (&state, &token) {
                    ...
                    (Ident, Token::Group(Delimiter::Parenthesis | Delimiter::Bracket, _)) => (false, Delim),
                    (Ident, Token::Punct('!', Spacing::Alone)) => (false, IdentBang),
                    (IdentBang, Token::Group(Delimiter::Parenthesis | Delimiter::Bracket, _)) => (false, Other),
    ...
    mod standard_library {
        ...
        use syn::{
            parenthesized, token, Attribute, Expr, ExprAssign, ExprPath, Ident, Lit, Macro, Pat, Path, Token, Type,
            Visibility,
        };
        enum KnownMacro {
            Expr(Expr),
        ...
        impl Printer {
            pub fn standard_library_macro(&mut self, mac: &Macro, mut semicolon: bool) -> bool {
                ...
                let parser = match name.as_str() {
                    ...
                    "const_format_args" | "eprint" | "eprintln" | "format" | "format_args" | "format_args_nl"
                    | "panic" | "print" | "println" | "todo" | "unimplemented" | "unreachable" => {
                        KnownMacro::parse_format_args
                    },
