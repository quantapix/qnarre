...
use rustc_ast::ast::expr::Limits;
use rustc_ast::ast::gen::bound::Modifier;
use rustc_ast::ast::gen::Gens;
use rustc_ast::ast::gen::Param;
use rustc_ast::ast::gen::Where;
use rustc_ast::ast::item::Use::Tree;
use rustc_ast::ast::lit::FloatType;
use rustc_ast::ast::lit::IntType;
use rustc_ast::ast::pat::Pat;
use rustc_ast::ast::stmt::Local;
use rustc_ast::ast::stmt::Stmt;
use rustc_ast::ast::typ::Type;
...
impl<T: SpanlessEq> SpanlessEq for ThinVec<T> {
    fn eq(&self, other: &Self) -> bool {
        self.len() == other.len() && self.iter().zip(other.iter()).all(|(a, b)| SpanlessEq::eq(a, b))
    }
}
impl<K: Eq + Hash, V: SpanlessEq, S: BuildHasher> SpanlessEq for HashMap<K, V, S> {
    fn eq(&self, other: &Self) -> bool {
        self.len() == other.len()
            && self
                .iter()
                .all(|(key, this_v)| other.get(key).map_or(false, |other_v| SpanlessEq::eq(this_v, other_v)))
    }
}
...
impl<A: SpanlessEq, B: SpanlessEq, C: SpanlessEq> SpanlessEq for (A, B, C) {
    fn eq(&self, other: &Self) -> bool {
        SpanlessEq::eq(&self.0, &other.0) && SpanlessEq::eq(&self.1, &other.1) && SpanlessEq::eq(&self.2, &other.2)
    }
}
...
spanless_eq_struct!(BareFnTy; unsafe_ ext generic_params decl decl_span);
spanless_eq_struct!(BindingAnnotation; 0 1);
spanless_eq_struct!(Block; stmts id rules span tokens could_be_bare_literal);
spanless_eq_struct!(Closure; binder capture_clause const_ asyncness movability fn_decl body !fn_decl_span !fn_arg_span);
...
spanless_eq_struct!(expr::Field; attrs id span ident expr is_shorthand is_placeholder);
spanless_eq_struct!(FieldDef; attrs id span vis ident ty is_placeholder);
spanless_eq_struct!(Fn; defaultness gens sig body);
spanless_eq_struct!(FnDecl; inputs output);
spanless_eq_struct!(FnHeader; const_ asyncness unsafe_ ext);
spanless_eq_struct!(FnSig; header decl span);
spanless_eq_struct!(Foreign; unsafe_ abi items);
...
spanless_eq_struct!(gen::Param; id ident attrs bounds is_placeholder kind !colon_span);
spanless_eq_struct!(gen::Gens; params where_clause span);
spanless_eq_struct!(Impl; defaultness unsafe_ gens const_ polarity of_trait self_ty items);
...
spanless_eq_struct!(Life; id ident);
spanless_eq_struct!(Lit; kind symbol suffix);
spanless_eq_struct!(stmt::Local; pat ty kind id span attrs !tokens);
...
spanless_eq_struct!(pat::Pat; id kind span tokens);
...
spanless_eq_struct!(stmt::Stmt; id kind span);
...
spanless_eq_struct!(Trait; unsafe_ is_auto gens bounds items);
spanless_eq_struct!(TraitRef; path ref_id);
spanless_eq_struct!(typ::Type; id kind span tokens);
...
spanless_eq_struct!(gen::Where; has_where_token predicates span);
spanless_eq_struct!(WhereEqPredicate; span lhs_ty rhs_ty);
spanless_eq_struct!(WhereRegionPredicate; span life bounds);
...
spanless_eq_enum!(FnRetTy; Default(0) typ::Type(0));
...
spanless_eq_enum!(GenericArg; Life(0) Type(0) Const(0));
spanless_eq_enum!(GenericArgs; AngleBracketed(0) Parenthesized(0));
spanless_eq_enum!(GenericBound; Trait(0 1) Outlives(0));
spanless_eq_enum!(GenericParamKind; Life Type(default) Const(ty kw_span default));
...
spanless_eq_enum!(lit::FloatType; Suffixed(0) Unsuffixed);
spanless_eq_enum!(lit::IntType; Signed(0) Unsigned(0) Unsuffixed);
...
spanless_eq_enum!(expr::Limits; HalfOpen Closed);
spanless_eq_enum!(StmtKind; stmt::Local(0) Item(0) Expr(0) Semi(0) Empty MacCall(0));
spanless_eq_enum!(StrStyle; Cooked Raw(0));
spanless_eq_enum!(StructRest; Base(0) Rest(0) None);
spanless_eq_enum!(Term; typ::Type(0) Const(0));
...
spanless_eq_enum!(InlineAsmOperand; In(reg expr) Out(reg late expr)
    ...
    Sym(sym));
spanless_eq_enum!(ItemKind; Extern(0) Use(0) Static(0) Const(0) Fn(0)
    ...
    fn eq(&self, other: &Self) -> bool {
        ...
        SpanlessEq::eq(id, id2)
            ...
            && (matches!(ty.kind, TyKind::Err)
                ...
                || SpanlessEq::eq(attrs, attrs2) && SpanlessEq::eq(ty, ty2) && SpanlessEq::eq(pat, pat2))
    }
}
impl SpanlessEq for TokenKind {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (TokenKind::Interpolated(this), TokenKind::Interpolated(other)) => match (this.as_ref(), other.as_ref()) {
                (Nonterminal::NtExpr(this), Nonterminal::NtExpr(other)) => SpanlessEq::eq(this, other),
                _ => this == other,
            },
            _ => self == other,
        }
...
impl SpanlessEq for AttrKind {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            (AttrKind::Normal(normal), AttrKind::Normal(normal2)) => SpanlessEq::eq(normal, normal2),
            ...
            (AttrKind::DocComment(kind, unescaped), AttrKind::Normal(normal2)) => {
                ...
                SpanlessEq::eq(&path, &normal2.item.path)
                    && match &normal2.item.args {
                        ...
                        AttrArgs::Eq(_span, value) => is_escaped_literal_attr_args(value, *unescaped),
                    }
            },
