...
- for justification, consider the following chain of conjectures (referenced lawyers are the divorce lawyer type):

- our general common law is adversarial, winner takes all
- a family by definition is not adversarial, both parents love their children
- however, with no conflict, obviously there is no lawsuit nor profits for lawyers
- thus common law applied to families must first turn a family into adversaries
- unsolved conflicts or perceived lack of resources create adversaries
- sustainable and intractable conflicts guarantee adversaries for life or "high-conflict"
- however, with no money nor possible profits, there simply cannot be "high-conflict"
- given sufficient funds, "high-conflict" cases are an ambitious lawyer's "gold mines" and long-term job-security
- lawyers are in abundance and competition is fierce as one only needs to be a convincing actor
- however, with no "high conflict", there are no trend-setting "interesting" cases
- with no "trend-setting", there is no fame nor $500 / hour billing rate for an ambitious lawyer

- accepting the above, what can an ambitious lawyer do? (other than change profession)
- he or she has only one choice: provoke or flat-out fabricate a conflict, where there is none, specifically for the Court's consumption
- in this blog we focus on lawyers directly fabricating conflicts, or flat-out, knowingly lying to the Court

- as per strict rules, all meaningful communication is (or can be converted to) textual

- our first goal thus is to train our computer to "catch the knowingly and directly lying lawyer"

- current NLP research has shown that the various proposed mechanism for answering the generic semantic correctness questions are promising
- we use them to train our elementary arithmetic model to tell us if a mathematical expression is correct or not

- YNS samples
- create samples

- there are still weaknesses regarding the size and scope of the considered "context" for correctness questions, however, our easily scaled and fully controlled "lab model" allows us to freely experiment

- the latest mt-dnn models achieve better than human performance due to "cross-training"

- we add variants to our samples:

- MSK masking inputs to quess
- REV reversing the order
- FIX to actually find the error 
...
- ABA ["frequent fliers"](https://www.americanbar.org/content/dam/aba/events/family_law/2014/05/section_of_familylawspringcleconference/11_fri_custody.authcheckdam.pdf)

- "the parents in conflict tend to be more preoccupied with their own identity reconstruction"

- NPD ["feminism"](https://owlcation.com/social-sciences/What-Judges-Need-to-Know-About-Narcissistic-Personality-Disorder-in-Custody-Cases)

The NPD will bring a plethora of legal actions that barely make sense or are fully nonsense. They will often burn through attorneys. They will have a history of multiple PFA’s against them. If they are male (and most are), they may have multiple custody issues with multiple women

- Psychology ["science"](https://www.theatlantic.com/science/archive/2018/11/psychologys-replication-crisis-real/576223/)

- In his foreword to my book, Alan Dershowitz discusses his time litigating cases in the old Soviet Union. He was always taken by the fact that they could prosecute anybody they wanted because some of the statutes were so vague
- Stalin ["vague"](https://www.cato.org/policy-report/januaryfebruary-2010/criminalization-almost-everything)
- most humans simply say no to such acts considering that children are involved
- there are humans, however, Nazi SS concentration-camp guards are quintessential examples, that will do the "job" as long as there is palatable excuse for them to "hate or despise the others"
- all
...
Impetus for this blog is provided by our parallel documenting efforts at [FemFas](https://femfas.net/) and particularly regarding the [actions](https://femfas.net/divorce_law/index.html) of our Family Court, summarized as: "Divorce law rigged to torture children", "Activist Family Court victimizing children", "Family Court knowingly forcing children into poverty" and "Court protecting, praising elite, white child predator men".

We continue to leave the clearly nebulous and tricky, day-to-day details of the situation to that website and we focus instead on actionable technical steps that can mold our still easily accessible "elementary arithmetic" model into the foundation of our upcoming text-based training efforts.

Further expanding on the last two blogs, we shift our attentions to the `classification` (CLS and CLX), `question-answer` (QAS), `compose` (GEN) groups of samples.    
