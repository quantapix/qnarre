...
def convert_data2vec_checkpoint_to_pytorch(
    data2vec_checkpoint_path, pytorch_dump_folder_path, classification_head
):
    ...
    data2vec.eval()  # disable drop
    data2vec_model = data2vec.models[0]
    data2vec_sent_encoder = data2vec_model.encoder.sentence_encoder
    config = Data2VecTextConfig(
        s_vocab=data2vec_sent_encoder.embed_tokens.num_embeddings,
        d_hidden=data2vec_model.args.encoder_embed_dim,
        n_lays=data2vec_model.args.encoder_layers,
        n_heads=data2vec_model.args.encoder_attention_heads,
        d_ff=data2vec_model.args.encoder_ffn_embed_dim,
        n_pos=514,
        n_typ=1,
        eps=1e-5,  # PyTorch default used in fairseq
    )
    ...
    for i in range(config.n_lays):
        ...
        ), f"Shape for data2vec_layer.self_attn.k_proj.weight.data should be {torch.Size((config.d_hidden, config.d_hidden))}"
        assert data2vec_layer.self_attn.q_proj.weight.data.shape == torch.Size(
            (config.d_hidden, config.d_hidden)
        ), f"Shape for data2vec_layer.self_attn.q_proj.weight.data should be {torch.Size((config.d_hidden, config.d_hidden))}"
        assert data2vec_layer.self_attn.v_proj.weight.data.shape == torch.Size(
            (config.d_hidden, config.d_hidden)
        ), f"Shape for data2vec_layer.self_attn.v_proj.weight.data should be {torch.Size((config.d_hidden, config.d_hidden))}"
    ...
    input_ids = data2vec.encode(SAMPLE_TEXT).unsqueeze(0)  # batch of size 1
