...
def convert_luke_checkpoint(
    checkpoint_path, metadata_path, entity_vocab_path, pytorch_dump_folder_path, model_size
):
    ...
    with open(
        os.path.join(pytorch_dump_folder_path, LukeTokenizer.vocab_fs["entity_vocab_file"]), "w"
    ) as f:
    ...
    for layer_index in range(config.n_lays):
