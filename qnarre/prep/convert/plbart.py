...
def convert_fairseq_plbart_checkpoint_from_disk(
    ...
    plbart_config = PLBartConfig.from_pretrained(hf_config_path, s_vocab=s_vocab)
    ...
    if not classification:
        ...
        for key, value in state_dict.copy().items():
            if key.startswith("classification_heads.sentence_classification_head"):
                classification_head[
                    key.replace("classification_heads.sentence_classification_head.", "")
                ] = value
                state_dict.pop(key)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--finetuned", action="store_true", help="whether the model is a fine-tuned checkpoint"
    )
    parser.add_argument(
        "--classification",
        action="store_true",
