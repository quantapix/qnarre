...
class TatoebaConverter:
    ...
    def convert_models(self, tatoeba_ids, dry_run=False):
        ...
        for model in tqdm(
            ...
            if not os.path.exists(save_dir / model["_name"]):
                download_and_unzip(
                    f"{TATOEBA_MODELS_URL}/{model['release']}", save_dir / model["_name"]
                )
    ...
    def resolve_lang_code(self, src, tgt):
    ...
    def write_model_card(self, model_dict, dry_run=False):
        """
        ...
        if "_data_per_pair" in model_dict:
            multilingual_data = (
                f"* data per pair in multilingual model: {model_dict['_data_per_pair']}\n"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-save_dir", "--save_dir", default="marian_converted", help="where to save converted models"
    )
