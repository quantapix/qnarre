...
class DPRState:
    ...
    @staticmethod
    def from_type(comp_type, *args, **kw):
        if comp_type.startswith("c"):
            return DPRContextEncoderState(*args, **kw)
        if comp_type.startswith("q"):
            return DPRQuestionEncoderState(*args, **kw)
        if comp_type.startswith("r"):
            return DPRReaderState(*args, **kw)
        else:
            raise ValueError(
...
def convert(comp_type, src_file: Path, dest_dir: Path):
