...
from ..config.marian import PreTrained
from ...run.marian import MTModel
...
def convert_encoder_layer(opus_dict, layer_prefix, converter):
    sd = {}
    for k in opus_dict:
...
def load_layers_(layer_lst: nn.ModuleList, opus_state, converter, is_dec=False):
    for i, layer in enumerate(layer_lst):
        layer_tag = f"decoder_l{i + 1}_" if is_dec else f"encoder_l{i + 1}_"
        sd = convert_encoder_layer(opus_state, layer_tag, converter)
        layer.load_state_dict(sd, strict=True)


def find_pretrained_model(src_lang, tgt_lang):
...
def add_emb_entries(wemb, final_bias, n_special_tokens=1):
    vsize, d_hidden = wemb.shape
    embs_to_add = np.zeros((n_special_tokens, d_hidden))
...
def cast_marian_config(raw_cfg):
...
def write_model_card(
    ...
    hf_model_name = remove_prefix(hf_model_name, ORG_NAME)
    opus_name = convert_hf_name_to_opus_name(hf_model_name)
...
def add_special_tokens_to_vocab(model_dir):
...
class OpusState:
    def __init__(self, source_dir, EOS=0):
        ...
        self.PAD = self.wemb.shape[0] - 1
        cfg["s_vocab"] = self.PAD + 1
        ...
        self.cfg = cfg
        d_hidden, intermediate_shape = self.state_dict["encoder_l1_ffn_W1"].shape
        if d_hidden != 512 or cfg["dim-emb"] != 512:
            raise ValueError(
                f"Hidden size {d_hidden} and configured size {cfg['dim_emb']} mismatched or not 512"
            )

        # Process decoder.yml
        decoder_yml = cast_marian_config(load_yaml(source_dir / "decoder.yml"))
        check_marian_cfg_assumptions(cfg)
        self.hf_config = PreTrained(
            s_vocab=cfg["s_vocab"],
            n_dec_lays=cfg["dec-depth"],
            n_enc_lays=cfg["enc-depth"],
            n_dec_heads=cfg["transformer-heads"],
            n_enc_heads=cfg["transformer-heads"],
            d_dec_ffn=cfg["transformer-dim-ffn"],
            d_enc_ffn=cfg["transformer-dim-ffn"],
            d_hidden=cfg["dim-emb"],
            act_fun=cfg["transformer-aan-activation"],
            PAD=self.PAD,
            EOS=EOS,
            forced_EOS=EOS,
            BOS=0,
            ...
            drop=0.1,  # see opus-mt-train repo/transformer-drop param.
            # default: add_final_norm=False,
            n_beams=decoder_yml["beam-size"],
            dec_START=self.PAD,
            bad_words_ids=[[self.PAD]],
            max_len=512,
        )
    ...
    def sub_keys(self, layer_prefix):
        return [
            remove_prefix(k, layer_prefix) for k in self.state_dict if k.startswith(layer_prefix)
        ]
    ...
    def load_marian_model(self):
        ...
        model = MTModel(cfg)
        if "d_hidden" in cfg.to_dict():
            raise ValueError("d_hidden is in config")
        load_layers_(
            model.model.encoder.layers,
            state_dict,
            BART_CONVERTER,
        )
        load_layers_(model.model.decoder.layers, state_dict, BART_CONVERTER, is_dec=True)
        ...
        if model.model.shared.padding_idx != self.PAD:
            raise ValueError(
                f"Padding tokens {model.model.shared.padding_idx} and {self.PAD} mismatched"
            )
        return model
...
def convert(source_dir, dest_dir):
    ...
    tokenizer_has_eos_token_id = hasattr(tokenizer, "EOS") and tokenizer.EOS is not None
    EOS = tokenizer.EOS if tokenizer_has_eos_token_id else 0
    opus_state = OpusState(source_dir, EOS=EOS)
    if opus_state.cfg["s_vocab"] != len(tokenizer.encoder):
        raise ValueError(
            f"Original vocab size {opus_state.cfg['s_vocab']} and new vocab size {len(tokenizer.encoder)} mismatched"
        )
...
if __name__ == "__main__":
    x = ArgumentParser()
    x.add_argument("--src", type=str, help="path to marian model sub dir", default="en-de")
    x.add_argument("--dest", type=str, default=None, help="Path to the output PyTorch model.")
    args = x.parse_args()
