...
from ..config.nystromformer import PreTrained
from ...models.nystromformer import ForMasked
...
def _helper(cfg, state):
    for key in state.copy().keys():
        val = state.pop(key)
        if ("pooler" in key) or ("sen_class" in key) or ("conv.bias" in key):
            continue
        else:
            state[rename_key(key)] = val
    state["cls.predictions.bias"] = state["cls.predictions.decoder.bias"]
    state["nystromformer.embeddings.position_ids"] = torch.arange(cfg.n_pos).expand((1, -1)) + 2
    return state
...
def to_pytorch(src_path, cfg_path, save_path):
    ...
    m = ForMasked(cfg)
    new_state_dict = _helper(cfg, orig_state_dict)
