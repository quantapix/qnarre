...
def configure_tpu(FLAGS):
    ...
    if FLAGS.use_tpu:
        ...
        strategy = tf.contrib.distribute.MirroredStrategy(num_gpus=FLAGS.num_core_per_host)
...
def get_train_op(FLAGS, total_loss, grads_and_vars=None):
    ...
    if FLAGS.weight_decay > 0 and not FLAGS.use_tpu and FLAGS.num_core_per_host > 1:
        raise ValueError("Do not support `weight_decay > 0` with multi-gpu " "training so far.")

    if FLAGS.weight_decay == 0:
        optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate, epsilon=FLAGS.adam_epsilon)
    else:
    ...
    if getattr(FLAGS, "lr_layer_decay_rate", 1.0) != 1.0:
        ...
        for i in range(len(clipped)):
            ...
            if not m:
                continue
            n_lays = max(n_lays, int(m.group(1)) + 1)

        for i in range(len(clipped)):
            for l in range(n_lays):
                if "model/transformer/layer_{}/".format(l) in variables[i].name:
                    abs_rate = FLAGS.lr_layer_decay_rate ** (n_lays - 1 - l)
                    clipped[i] *= abs_rate
                    tf.logging.info(
...
def clean_ckpt(_):
    ...
    with tf.variable_scope(tf.get_variable_scope(), reuse=tf.AUTO_REUSE):
        tf_vars = [
            tf.get_variable(v, shape=var_values[v].shape, dtype=var_dtypes[v]) for v in var_values
        ]
    ...
    with tf.Session() as sess:
        ...
        saver.save(sess, join(output_model_dir, "model.ckpt"), global_step=global_step)
