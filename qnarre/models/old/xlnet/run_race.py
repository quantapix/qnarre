...
flags.DEFINE_integer("train_steps", default=12000, help="Number of training steps")
...
flags.DEFINE_integer("eval_batch_size", default=32, help="Batch size for evaluation.")

# Data config
flags.DEFINE_integer("max_seq_length", default=512, help="Max length for the paragraph.")
flags.DEFINE_integer(
    "max_qa_length", default=128, help="Max length for the concatenated question and answer."
)
...
def convert_single_example(example, tokenize_fn):
    ...
    feature = InputFeatures(
        input_ids=input_ids, input_mask=input_mask, segment_ids=all_seg_ids, label_id=label_id
    )
    return feature
...
def main(_):
    ...
    if FLAGS.do_train:
        ...
        if not tf.gfile.Exists(train_file) or FLAGS.overwrite_data:
            ...
            file_based_convert_examples_to_features(train_examples, tokenize_fn, train_file)
    ...
    if FLAGS.do_eval:
        ...
        file_based_convert_examples_to_features(eval_examples, tokenize_fn, eval_file)
