...
parser.add_argument(
    "--data", type=str, default="../data/wikitext-103", help="location of the data corpus"
)
parser.add_argument(
    "--dataset",
    type=str,
...
parser.add_argument("--n_lays", type=int, default=12, help="number of total layers")
parser.add_argument("--n_heads", type=int, default=10, help="number of heads")
...
parser.add_argument("--drop", type=float, default=0.0, help="global drop rate")
parser.add_argument("--dropatt", type=float, default=0.0, help="attention probability drop rate")
...
parser.add_argument(
    "--not_tied", action="store_true", help="do not tie the word embedding and softmax weights"
)
...
parser.add_argument(
    "--fp16", action="store_true", help="Run in pseudo-fp16 mode (fp16 storage fp32 math)."
)
parser.add_argument(
    "--static-loss-scale",
    type=float,
...
args.n_nonemb_param = sum([p.nelement() for p in model.lays.parameters()])
...
if args.multi_gpu:
    ...
    if args.gpu0_bsz >= 0:
        para_model = BalancedDataParallel(args.gpu0_bsz // args.batch_chunk, model, dim=1).to(
            device
        )
    else:
...
if args.optim.lower() == "sgd":
    if args.sample_softmax > 0:
        ...
        for param in model.parameters():
            if param.size() == model.tok_emb.weight.size():
...
if args.scheduler == "cosine":
    ...
    def lr_lambda(step):
        ...
        if step == 0 and args.warmup_step == 0:
            ...
            return (
                1.0 / (step**0.5) if step > args.warmup_step else step / (args.warmup_step**1.5)
            )
...
def evaluate(eval_iter):
    # Turn on evaluation mode which disables drop.
    model.eval()
...
def train():
    # Turn on training mode which enables drop.
