...
def create_model(
    ...
    d_hidden = output_layer.shape[-1].value
    ...
    with tf.variable_scope("loss"):
        if is_training:
            # I.e., 0.1 drop
            output_layer = tf.nn.drop(output_layer, keep_prob=0.9)
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for (i, prediction) in enumerate(result):
                ...
                if i >= num_actual_predict_examples:
                    break
                output_line = (
                    "\t".join(str(class_probability) for class_probability in probabilities) + "\n"
                )
