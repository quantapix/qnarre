...
class MultiheadAttentionLayer(nn.Module):
    ...
    def __init__(self, config, device="cpu", dtype=torch.float32):
        ...
        self.resid_drop = nn.Dropout(config.drop_embed)
        self.c_proj = nn.Linear(config.n_hidden, config.n_hidden, device=device, dtype=dtype)
        ...
        self.attn = torch.nn.MultiheadAttention(
            embed_dim=config.n_hidden,
            num_heads=config.n_head,
            dropout=config.drop_attn,
            batch_first=True,
            device=device,
            dtype=dtype,
        )
...
class Block(nn.Module):
    ...
    def __init__(self, config: GPTConfig):
        super().__init__()
        self.ln1 = nn.LayerNorm(config.n_hidden)
...
class EmbeddingStem(nn.Module):
    def __init__(self, config: GPTConfig, device="cpu", dtype=torch.float32):
        super().__init__()
        self.tok_emb = nn.Embedding(config.s_vocab, config.n_hidden, device=device, dtype=dtype)
        self.pos_emb = nn.Parameter(
            torch.zeros(1, config.block_size, config.n_hidden, device=device, dtype=dtype)
        )
        self.drop = nn.Dropout(config.drop_embed)
...
class GPT(nn.Module):
    ...
    def __init__(self, config: GPTConfig):
        ...
        # decoder head
        self.ln_f = nn.LayerNorm(config.n_hidden)
        self.head = nn.Linear(config.n_hidden, config.s_vocab, bias=False)
    ...
    def _set_model_config(self, config):
        ...
        params_given = all(
            [config.n_layer is not None, config.n_head is not None, config.n_hidden is not None]
        )
            ...
            config.__dict__.update(
                {
                    ...
                    # GPT-1
                    "openai-gpt": dict(n_layer=12, n_head=12, n_hidden=768),  # 117M params
                    # GPT-2 configs
                    ...
                    # Gophers
                    "gopher-44m": dict(n_layer=8, n_head=16, n_hidden=512),
                    # (there are a number more...)
                    # I made these tiny models up
                    "gpt-mini": dict(n_layer=6, n_head=6, n_hidden=192),
                    "gpt-micro": dict(n_layer=4, n_head=4, n_hidden=128),
                    "gpt-nano": dict(n_layer=3, n_head=3, n_hidden=48),
                }[config.model_type]
            )
        return config
