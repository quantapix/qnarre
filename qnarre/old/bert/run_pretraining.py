    ...
    with tf.variable_scope("cls/predictions"):
        ...
        with tf.variable_scope("transform"):
            input_tensor = tf.layers.dense(
                input_tensor,
                units=bert_config.d_hidden,
                activation=modeling.get_activation(bert_config.act),
                kernel_initializer=modeling.create_initializer(bert_config.init_range),
            )
        ...
        output_bias = tf.get_variable(
            "output_bias", shape=[bert_config.s_vocab], initializer=tf.zeros_initializer()
        )
        ...
        one_hot_labels = tf.one_hot(label_ids, depth=bert_config.s_vocab, dtype=tf.float32)
...
def get_next_sentence_output(bert_config, input_tensor, labels):
    ...
    with tf.variable_scope("cls/seq_relationship"):
        output_weights = tf.get_variable(
            "output_weights",
            shape=[2, bert_config.d_hidden],
            initializer=modeling.create_initializer(bert_config.init_range),
        )
