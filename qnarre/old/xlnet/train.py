...
flags.DEFINE_bool("use_tpu", default=True, help="Use TPUs rather than plain CPUs.")
...
flags.DEFINE_bool("track_mean", default=False, help="Whether to track mean loss.")
...
flags.DEFINE_integer(
    "train_batch_size", default=16, help="Size of the train batch across all hosts."
)
...
flags.DEFINE_integer(
    "save_steps",
    default=None,
    help="Number of steps for model checkpointing. " "None for not saving checkpoints",
)
flags.DEFINE_integer("max_save", default=100000, help="Maximum number of checkpoints to save.")
...
def get_cache_fn(mem_len):
    ...
    def cache_fn(batch_size):
        mems = []
        if FLAGS.mem_len > 0:
            for _ in range(FLAGS.n_lays):
                zeros = tf.zeros([mem_len, batch_size, FLAGS.d_model], dtype=tf_float)
                mems.append(zeros)
...
def main(unused_argv):
    ...
    tf.logging.info("num of batches {}".format(train_record_info_dict["num_batch"]))
