...
def construct_scalar_host_call(monitor_dict, model_dir, prefix="", reduce_fn=None):
    """
...
def get_classification_loss(FLAGS, features, n_class, is_training):
    ...
    summary = xlnet_model.get_pooled_out(FLAGS.sum_type, FLAGS.use_summ_proj)
...
def get_qa_outputs(FLAGS, features, is_training):
    ...
    with tf.variable_scope("start_logits"):
        start_logits = tf.layers.dense(output, 1, kernel_initializer=initializer)
    ...
    with tf.variable_scope("end_logits"):
        if is_training:
            ...
            end_logits = tf.layers.dense(
                end_logits, 1, kernel_initializer=initializer, name="dense_1"
            )
            ...
            start_top_log_probs, start_top_index = tf.nn.top_k(start_log_probs, k=FLAGS.start_n_top)
            ...
            end_input = tf.tile(output[:, :, None], [1, 1, FLAGS.start_n_top, 1])
            ...
            end_log_probs = tf.nn.log_softmax(end_logits_masked, -1)
            end_top_log_probs, end_top_index = tf.nn.top_k(end_log_probs, k=FLAGS.end_n_top)
            end_top_log_probs = tf.reshape(
                end_top_log_probs, [-1, FLAGS.start_n_top * FLAGS.end_n_top]
            )
            end_top_index = tf.reshape(end_top_index, [-1, FLAGS.start_n_top * FLAGS.end_n_top])
    ...
    with tf.variable_scope("answer_class"):
        ...
        ans_feature = tf.layers.drop(ans_feature, FLAGS.drop, training=is_training)
        cls_logits = tf.layers.dense(
            ans_feature, 1, kernel_initializer=initializer, name="dense_1", use_bias=False
        )
...
def get_race_loss(FLAGS, features, is_training):
    ...
    with tf.variable_scope("logits"):
        logits = tf.layers.dense(summary, 1, kernel_initializer=xlnet_model.get_initializer())
