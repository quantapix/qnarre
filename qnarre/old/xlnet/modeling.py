...
def embedding_lookup(
    ...
    with tf.variable_scope(scope, reuse=reuse):
        lookup_table = tf.get_variable(
            "lookup_table", [n_token, d_embed], dtype=dtype, initializer=initializer
        )
        if use_tpu:
...
def positionwise_ffn(
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        output = tf.layers.drop(output, drop, training=is_training, name="drop_1")
        output = tf.layers.dense(
            output, d_model, kernel_initializer=kernel_initializer, name="layer_2"
        )
        output = tf.layers.drop(output, drop, training=is_training, name="drop_2")
        output = tf.contrib.layers.layer_norm(output + inp, begin_norm_axis=-1, scope="LayerNorm")
    return output


def head_projection(h, d_model, n_heads, d_head, kernel_initializer, name):
...
def post_attention(
    h, attn_vec, d_model, n_heads, d_head, drop, is_training, kernel_initializer, residual=True
):
    """Post-attention processing."""
    # post-attention projection (back to `d_model`)
    proj_o = tf.get_variable(
        "o/kernel", [d_model, n_heads, d_head], dtype=h.dtype, initializer=kernel_initializer
    )
    attn_out = tf.einsum("ibnd,hnd->ibh", attn_vec, proj_o)

    attn_out = tf.layers.drop(attn_out, drop, training=is_training)
    if residual:
...
def abs_attn_core(q_head, k_head, v_head, attn_mask, dropatt, is_training, scale):
    ...
    attn_prob = tf.layers.drop(attn_prob, dropatt, training=is_training)
...
def multihead_attn(
    ...
    with tf.variable_scope(scope, reuse=reuse):
        # attention heads
        q_head = head_projection(q, d_model, n_heads, d_head, kernel_initializer, "q")
        k_head = head_projection(k, d_model, n_heads, d_head, kernel_initializer, "k")
        v_head = head_projection(v, d_model, n_heads, d_head, kernel_initializer, "v")
...
def rel_multihead_attn(
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # content heads
        q_head_h = head_projection(h, d_model, n_heads, d_head, kernel_initializer, "q")
        k_head_h = head_projection(cat, d_model, n_heads, d_head, kernel_initializer, "k")
        v_head_h = head_projection(cat, d_model, n_heads, d_head, kernel_initializer, "v")

        # positional heads
        k_head_r = head_projection(r, d_model, n_heads, d_head, kernel_initializer, "r")
        ...
        # post processing
        output = post_attention(
            h, attn_vec, d_model, n_heads, d_head, drop, is_training, kernel_initializer
        )
...
def two_stream_rel_attn(
    ...
    with tf.variable_scope(scope, reuse=False):
        ...
        output_h = post_attention(
            h, attn_vec_h, d_model, n_heads, d_head, drop, is_training, kernel_initializer
        )

    with tf.variable_scope(scope, reuse=True):
        ##### g-stream
        # query-stream query head
        q_head_g = head_projection(g, d_model, n_heads, d_head, kernel_initializer, "q")
        ...
        output_g = post_attention(
            g, attn_vec_g, d_model, n_heads, d_head, drop, is_training, kernel_initializer
        )
    ...
    with tf.variable_scope(scope):
        if untie_r:
            r_w_bias = tf.get_variable(
                "r_w_bias", [n_lays, n_heads, d_head], dtype=tf_float, initializer=initializer
            )
            r_r_bias = tf.get_variable(
                "r_r_bias", [n_lays, n_heads, d_head], dtype=tf_float, initializer=initializer
            )
        else:
            r_w_bias = tf.get_variable(
                "r_w_bias", [n_heads, d_head], dtype=tf_float, initializer=initializer
            )
            r_r_bias = tf.get_variable(
                "r_r_bias", [n_heads, d_head], dtype=tf_float, initializer=initializer
            )
        ...
        if attn_mask is not None:
            ...
            non_tgt_mask = tf.concat(
                [tf.zeros([qlen, mlen], dtype=tf_float), non_tgt_mask], axis=-1
            )
        ...
        output_h = tf.layers.drop(word_emb_k, drop, training=is_training)
        if inp_q is not None:
            output_g = tf.layers.drop(word_emb_q, drop, training=is_training)

        ##### Segment embedding
        if seg_id is not None:
            if untie_r:
                r_s_bias = tf.get_variable(
                    "r_s_bias", [n_lays, n_heads, d_head], dtype=tf_float, initializer=initializer
                )
            else:
                # default case (tie)
                r_s_bias = tf.get_variable(
                    "r_s_bias", [n_heads, d_head], dtype=tf_float, initializer=initializer
                )

            seg_embed = tf.get_variable(
                "seg_embed", [n_lays, 2, n_heads, d_head], dtype=tf_float, initializer=initializer
            )
        ...
        pos_emb = relative_positional_encoding(
            qlen, klen, d_model, clamp_len, attn_type, bi_data, bsz=bsz, dtype=tf_float
        )
        pos_emb = tf.layers.drop(pos_emb, drop, training=is_training)
        ...
        if inp_q is not None:
            output = tf.layers.drop(output_g, drop, training=is_training)
        else:
            output = tf.layers.drop(output_h, drop, training=is_training)
    ...
    with tf.variable_scope("lm_loss"):
        if tie_weight:
            ...
            softmax_w = tf.get_variable(
                "weight", [n_token, d_model], dtype=hidden.dtype, initializer=initializer
            )

        softmax_b = tf.get_variable(
            "bias", [n_token], dtype=hidden.dtype, initializer=tf.zeros_initializer()
        )
    ...
    with tf.variable_scope(scope, "sequnece_summary", reuse=reuse):
        if sum_type == "last":
            ...
            summary_bias = tf.get_variable(
                "summary_bias", [d_model], dtype=hidden.dtype, initializer=initializer
            )
            ...
            raise ValueError("Unsupported summary type {}".format(sum_type))

        # use another projection as in BERT
        if use_proj:
            summary = tf.layers.dense(
                summary, d_model, activation=tf.tanh, kernel_initializer=initializer, name="summary"
            )

        # drop
        summary = tf.layers.drop(summary, drop, training=is_training, name="drop")

    return summary


def classification_loss(
    hidden, labels, n_class, initializer, scope, reuse=None, return_logits=False
):
    """
