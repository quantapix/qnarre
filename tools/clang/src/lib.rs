...
#![cfg_attr(
    ...
    allow(clippy::unreadable_literal, clippy::missing_safety_doc, clippy::too_many_arguments)
)]
...
fn find(path: &Path, patterns: &[&str]) -> Option<PathBuf> {
    let path = if let Some(x) = path.to_str() {
        Path::new(&Pattern::escape(x)).to_owned()
    } else {
        return None;
    };
    for p in patterns {
        let p = path.join(p).to_string_lossy().into_owned();
