...
impl Expr {
    ...
    pub fn replace_attrs(&mut self, y: Vec<attr::Attr>) -> Vec<attr::Attr> {
        match self {
            Expr::Array(Array { attrs, .. })
            | Expr::Assign(Assign { attrs, .. })
            | Expr::Async(Async { attrs, .. })
            | Expr::Await(Await { attrs, .. })
            | Expr::Binary(Binary { attrs, .. })
            | Expr::Block(Block { attrs, .. })
            | Expr::Break(Break { attrs, .. })
            | Expr::Call(Call { attrs, .. })
            | Expr::Cast(Cast { attrs, .. })
            | Expr::Closure(Closure { attrs, .. })
            | Expr::Const(Const { attrs, .. })
            | Expr::Continue(Continue { attrs, .. })
            | Expr::Field(Field { attrs, .. })
            | Expr::For(For { attrs, .. })
            | Expr::Group(Group { attrs, .. })
            | Expr::If(If { attrs, .. })
            | Expr::Index(Index { attrs, .. })
            | Expr::Infer(Infer { attrs, .. })
            | Expr::Let(Let { attrs, .. })
            | Expr::Lit(Lit { attrs, .. })
            | Expr::Loop(Loop { attrs, .. })
            | Expr::Mac(Mac { attrs, .. })
            | Expr::Match(Match { attrs, .. })
            | Expr::Method(Method { attrs, .. })
            | Expr::Parenth(Parenth { attrs, .. })
            | Expr::Path(Path { attrs, .. })
            | Expr::Range(Range { attrs, .. })
            | Expr::Ref(Ref { attrs, .. })
            | Expr::Repeat(Repeat { attrs, .. })
            | Expr::Return(Return { attrs, .. })
            | Expr::Struct(Struct { attrs, .. })
            | Expr::Try(Try { attrs, .. })
            | Expr::TryBlock(TryBlock { attrs, .. })
            | Expr::Tuple(Tuple { attrs, .. })
            | Expr::Unary(Unary { attrs, .. })
            | Expr::Unsafe(Unsafe { attrs, .. })
            | Expr::While(While { attrs, .. })
            | Expr::Yield(Yield { attrs, .. }) => std::mem::replace(attrs, y),
    ...
    pub fn is_blocklike(&self) -> bool {
        match self {
            ...
            | Expr::TryBlock(TryBlock { attrs, .. })
            | Expr::Tuple(Tuple { attrs, .. })
            | Expr::Unsafe(Unsafe { attrs, .. }) => !attr::has_outer(attrs),
            Expr::Assign(_)
            | Expr::Await(_)
            | Expr::Binary(_)
            | Expr::Break(_)
    ...
    fn has_struct_lit(&self) -> bool {
        match self {
            Expr::Struct(_) => true,
            Expr::Assign(Assign { left, right, .. }) | Expr::Binary(Binary { left, right, .. }) => {
            ...
            | Expr::Method(Method { expr, .. })
            | Expr::Ref(Ref { expr, .. })
            | Expr::Unary(Unary { expr, .. }) => expr.has_struct_lit(),
            Expr::Array(_)
            | Expr::Async(_)
            | Expr::Block(_)
            | Expr::Break(_)
    ...
    fn needs_newline(&self) -> bool {
        match self {
            ...
            | Expr::Match(_)
            | Expr::Path(_)
            | Expr::Range(Range { end: None, .. })
            | Expr::Repeat(_)
            | Expr::Return(Return { expr: None, .. })
            | Expr::Struct(_)
            | Expr::TryBlock(_)
            | Expr::Tuple(_)
            | Expr::Unsafe(_)
            | Expr::Verbatim(_)
            | Expr::While(_)
            | Expr::Yield(Yield { expr: None, .. }) => false,
            Expr::Assign(_)
            | Expr::Await(_)
            | Expr::Binary(_)
            | Expr::Cast(_)
            ...
            | Expr::Index(_)
            | Expr::Method(_) => true,
            Expr::Break(Break { val: Some(x), .. })
            | Expr::Call(Call { func: x, .. })
            | Expr::Group(Group { expr: x, .. })
            | Expr::Let(Let { expr: x, .. })
            | Expr::Parenth(Parenth { expr: x, .. })
            | Expr::Range(Range { end: Some(x), .. })
            | Expr::Ref(Ref { expr: x, .. })
            | Expr::Return(Return { expr: Some(x), .. })
            | Expr::Try(Try { expr: x, .. })
            | Expr::Unary(Unary { expr: x, .. })
            | Expr::Yield(Yield { expr: Some(x), .. }) => x.needs_newline(),
        }
    }
