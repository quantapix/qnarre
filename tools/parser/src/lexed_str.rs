...
impl<'a> LexedStr<'a> {
    ...
    pub fn single_token(text: &'a str) -> Option<(SyntaxKind, Option<String>)> {
        ...
        match &*conv.res.kinds {
            [x] => Some((*x, conv.res.errs.pop().map(|x| x.msg))),
            _ => None,
        }
    }
    ...
    pub fn error(&self, i: usize) -> Option<&str> {
        assert!(i < self.len());
        let err = self.errs.binary_search_by_key(&(i as u32), |i| i.token).ok()?;
        Some(self.errs[err].msg.as_str())
    }
    pub fn errors(&self) -> impl Iterator<Item = (usize, &str)> + '_ {
        self.errs.iter().map(|x| (x.token as usize, x.msg.as_str()))
    }
...
impl<'a> Converter<'a> {
    ...
    fn push(&mut self, kind: SyntaxKind, len: usize, err: Option<&str>) {
        ...
        if let Some(err) = err {
            ...
            self.res.errs.push(LexError { msg, token });
        }
    }
