...
use crate::{format_to, SyntaxKind, T};
    ...
    mod reparsing {
        use crate::{
            syntax::{
                ...
                parsing::build_tree,
                Indel, SyntaxErr, TextRange, TextSize,
            },
            Lexed, Reparser,
            SyntaxKind::*,
            T,
        ...
        fn reparse_token(root: &syntax::Node, edit: &Indel) -> Option<(green::Node, Vec<SyntaxErr>, TextRange)> {
            ...
            match prev_token_kind {
                WHITESPACE | COMMENT | IDENT | STRING | BYTE_STRING | C_STRING => {
                    ...
                    let (new_token_kind, new_err) = Lexed::single_token(&new_text)?;
                    ...
                    if let Some(next_char) = root.text().char_at(prev_token.text_range().end()) {
                        ...
                        let token_with_next_char = Lexed::single_token(&new_text);
        ...
        fn reparse_block(root: &syntax::Node, edit: &Indel) -> Option<(green::Node, Vec<SyntaxErr>, TextRange)> {
            let (node, reparser) = find_reparsable_node(root, edit.delete)?;
            let text = get_text_after_edit(node.clone().into(), edit);
            let lexed = Lexed::new(text.as_str());
        ...
        fn get_text_after_edit(element: syntax::Elem, edit: &Indel) -> String {
        ...
        fn find_reparsable_node(node: &syntax::Node, range: TextRange) -> Option<(syntax::Node, Reparser)> {
        ...
        fn is_balanced(lexed: &Lexed<'_>) -> bool {
...
",
    ...
    pub fn parse_text(text: &str) -> (green::Node, Vec<SyntaxErr>) {
        ...
        let parser_input = lexed.to_input();
        let parser_output = TopEntry::SourceFile.parse(&parser_input);
        let (node, errors, _eof) = build_tree(lexed, parser_output);
        (node, errors)
    }
    pub fn build_tree(lexed: Lexed<'_>, parser_output: Output) -> (green::Node, Vec<SyntaxErr>, bool) {
        ...
        let is_eof = lexed.intersperse_trivia(&parser_output, &mut |step| match step {
            ...
            StrStep::Error { msg, pos } => builder.error(msg.to_string(), pos.try_into().unwrap()),
        });
...
mod ptr {
    ...
    impl<N: ast::Node> AstPtr<N> {
        ...
        pub fn to_node(&self, root: &syntax::Node) -> N {
...
pub mod ted {
    ...
    pub trait Elem {
        fn syntax_element(self) -> syntax::Elem;
    }
    ...
    impl Elem for syntax::Elem {
        fn syntax_element(self) -> syntax::Elem {
            self
        }
    }
    impl Elem for syntax::Node {
        fn syntax_element(self) -> syntax::Elem {
            self.into()
        }
    }
    impl Elem for syntax::Token {
        fn syntax_element(self) -> syntax::Elem {
            self.into()
        }
    }
    ...
    impl Pos {
        ...
        pub fn first_child_of(x: &(impl Into<syntax::Node> + Clone)) -> Pos {
            let repr = PosRepr::FirstChild(x.clone().into());
            Pos { repr }
        }
        pub fn last_child_of(x: &(impl Into<syntax::Node> + Clone)) -> Pos {
            let x = x.clone().into();
    ...
    pub fn insert_all(pos: Pos, mut xs: Vec<syntax::Elem>) {
    ...
    pub fn insert_all_raw(pos: Pos, xs: Vec<syntax::Elem>) {
    ...
    pub fn remove_all(range: RangeInclusive<syntax::Elem>) {
        replace_all(range, Vec::new());
    }
    pub fn remove_all_iter(range: impl IntoIterator<Item = syntax::Elem>) {
    ...
    pub fn replace_with_many(old: impl Elem, new: Vec<syntax::Elem>) {
        let old = old.syntax_element();
        replace_all(old.clone()..=old, new);
    }
    pub fn replace_all(range: RangeInclusive<syntax::Elem>, new: Vec<syntax::Elem>) {
        let start = range.start().index();
        let end = range.end().index();
        let parent = range.start().parent().unwrap();
        parent.splice_children(start..end + 1, new);
    }
    pub fn append_child(x: &(impl Into<syntax::Node> + Clone), child: impl Elem) {
        let pos = Pos::last_child_of(x);
        insert(pos, child);
    }
    pub fn append_child_raw(x: &(impl Into<syntax::Node> + Clone), child: impl Elem) {
        let pos = Pos::last_child_of(x);
        insert_raw(pos, child);
    }
    fn ws_before(pos: &Pos, new: &syntax::Elem) -> Option<syntax::Token> {
    ...
    fn ws_after(pos: &Pos, new: &syntax::Elem) -> Option<syntax::Token> {
    ...
    fn ws_between(left: &syntax::Elem, right: &syntax::Elem) -> Option<syntax::Token> {
...
#[test]
fn api_walkthrough() {
    use crate::syntax::core::{NodeOrToken, WalkEvent};
