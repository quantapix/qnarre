use crate::{
    format_to,
    syntax::{self, ast, core::NodeOrToken, utils::is_raw_identifier, SourceFile},
    SyntaxKind,
};
...
pub fn hacky_block_expr(
    elements: impl IntoIterator<Item = syntax::Elem>,
    tail_expr: Option<ast::Expr>,
) -> ast::BlockExpr {
    let mut buf = "{\n".to_string();
...
pub fn expr_if(condition: ast::Expr, then_branch: ast::BlockExpr, else_branch: Option<ast::ElseBranch>) -> ast::Expr {
...
pub fn expr_method_call(receiver: ast::Expr, method: ast::NameRef, arg_list: ast::ArgList) -> ast::Expr {
...
pub fn struct_(
    ...
    let semicolon = if matches!(field_list, ast::FieldList::TupleFieldList(_)) {
        ";"
    } else {
        ""
    };
...
pub fn token(kind: SyntaxKind) -> syntax::Token {
    tokens::SOURCE_FILE
        .tree()
        .syntax()
        .clone_for_update()
...
pub mod tokens {
    use crate::{
        syntax::{self, ast, Parse, SourceFile},
        SyntaxKind::*,
    };
    ...
    pub fn single_space() -> syntax::Token {
        SOURCE_FILE
            .tree()
            .syntax()
            .clone_for_update()
    ...
    pub fn whitespace(text: &str) -> syntax::Token {
    ...
    pub fn doc_comment(text: &str) -> syntax::Token {
        assert!(!text.trim().is_empty());
        let sf = SourceFile::parse(text).ok().unwrap();
        sf.syntax().first_child_or_token().unwrap().into_token().unwrap()
    }
    pub fn literal(text: &str) -> syntax::Token {
        assert_eq!(text.trim(), text);
        let lit: ast::Literal = super::ast_from_text(&format!("fn f() {{ let _ = {text}; }}"));
        lit.syntax().first_child_or_token().unwrap().into_token().unwrap()
    }
    pub fn single_newline() -> syntax::Token {
        let res = SOURCE_FILE
            .tree()
            .syntax()
            .clone_for_update()
    ...
    pub fn blank_line() -> syntax::Token {
        SOURCE_FILE
            .tree()
            .syntax()
            .clone_for_update()
