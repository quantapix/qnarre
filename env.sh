...
spacy() {
    if $1; then
        .env/bin/pip install spacy[lookups,transformers,cuda115]
    else
...
ptorch() {
    if $1; then
        .env/bin/pip install -U cupy-cuda115
        .env/bin/pip install --pre torch torchtext torchvision torchaudio -f https://download.pytorch.org/whl/nightly/cu115/torch_nightly.html
    else
        .env/bin/pip install --pre torch torchtext torchvision torchaudio -f https://download.pytorch.org/whl/nightly/cpu/torch_nightly.html
    fi
